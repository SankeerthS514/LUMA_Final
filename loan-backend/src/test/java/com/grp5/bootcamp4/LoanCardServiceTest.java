...
class LoanCardServiceTest {
	...
	public void getLoansTest() {
		...
		Mockito.when(loanRepository.findAll()).thenReturn(LoanCards);
		Assert.assertEquals(2,loanService.getAllLoan().size());
	}
	...
	public void getLoanByIdTest() throws CustomErrorMessage {
		...
		Mockito.when(loanRepository.findById(id2).get()).thenReturn(
				returnLoan);
		Assert.assertEquals(1,loanService.getLoanById(id2).getId());
	}
//	
//	@Test
